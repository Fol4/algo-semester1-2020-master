...
public:
	...
	int64_t search(int64_t val) {
		Node* cur = root;

		while (true) {

			if (cur->number - is_number(cur->left) - 1 == val) {
				break;
			}
			else if (val < is_number(cur->right)) {
				cur = cur->right;
			}
			else {
				val -= cur->number - is_number(cur->left);
				cur = cur->left;
			}
		}
...
int main() {
	...
	for (int64_t i = 0; i < n; ++i) {
		...
		result.push_back(tree.search(count - k - 1));
	}
